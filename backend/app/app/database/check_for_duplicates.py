...
for mem in res:
    ...
    sql_query = select(OrganizationMembership).where(OrganizationMembership.person == mem.person).where(OrganizationMembership.organization == mem.organization).where(OrganizationMembership.start_date == mem.start_date).where(OrganizationMembership.end_date == mem.end_date).where(OrganizationMembership.id != mem.id)
...
res = sess.exec(sql_query)
for com in res:
    sql_query = select(Communications).where(Communications.party_1 == com.party_1).where(Communications.party_2 == com.party_2).where(Communications.com_date == com.com_date).where(Communications.topic == com.topic).where(Communications.id != com.id)
    matches = sess.exec(sql_query).all()
    if len(matches) >= 1:
