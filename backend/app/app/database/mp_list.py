...
for file in file_list:
    ...
    for c in caucus_roles:
        sql_query = select(Organization.id).where(Organization.match_name == create_match_name(c['CaucusShortName']))
        res = sess.exec(sql_query).first()
        if res is None:
            cauc = Organization(display_name=c['CaucusShortName'],
        ...
        sql_query = select(OrganizationMembership.id).where(OrganizationMembership.person == person_id).where(OrganizationMembership.organization == cauc_id)
        res = sess.exec(sql_query).first()
        if res is None:
            cauc_mem = OrganizationMembership(start_date=sd, end_date=ed, person=person_id, organization=cauc_id, source=src_id)
            sess.add(cauc_mem)
            sess.commit()

    for p in parliamentary_positions:
        try:
            sql_query = select(Organization.id).where(Organization.match_name == create_match_name(p['Title']))
            res = sess.exec(sql_query).first()
            if res is None:
                pp = Organization(display_name=p['Title'], match_name=create_match_name(p['Title']),
                                  organization_type=gov_org_type_id, sector=gov_sect_id, parent_organization=fed_gov_id, source=src_id)
                sess.add(pp)
                sess.commit()
                pp_id = pp.id
            else:
            ...
            sql_query = select(OrganizationMembership.id).where(OrganizationMembership.person == person_id).where(OrganizationMembership.organization == pp_id)
            res = sess.exec(sql_query)
            if res is None:
                pp_mem = OrganizationMembership(person=person_id, organization=pp_id, start_date=sd, end_date=ed, source=src_id)
                sess.add(pp_mem)
                sess.commit()
        except:
            pass
