...
def aggregate_parallel_edges(g):
    ...
    # get parallel edges
    fund_parallel_edge_node_pairs = {"FUNDS": {},
                                     "COMMUNICATION": {},
                                     "MEMBERSHIP": {}}
            ...
            for dst in adj_mat[src].keys():
                if len(adj_mat[src][dst]) > 1:  # this will be the parallel edge
                    ...
                    for e_type in fund_parallel_edge_node_pairs.keys():
                        ...
                        filtered_adjacency = filter_edge_by_type(adj_mat[src][dst], e_type)
                        ...
                        if k not in fund_parallel_edge_node_pairs[e_type].keys():
                            fund_parallel_edge_node_pairs[e_type][k] = {'amount': 0,
                                                                        'e_keys': [],
                                                                        'dash': None,
                                                                        "color": None}
                        ...
                        for r in filtered_adjacency.keys():
                            fund_parallel_edge_node_pairs[e_type][k]['amount'] += filtered_adjacency[r][
                                'amount']
                            fund_parallel_edge_node_pairs[e_type][k]['e_keys'].append(r)
                            fund_parallel_edge_node_pairs[e_type][k]['dash'] = filtered_adjacency[r]['dash']
                            fund_parallel_edge_node_pairs[e_type][k]['color'] = filtered_adjacency[r]['color']
                            ...
                            fund_parallel_edge_node_pairs[e_type][k]['start_date'] = early_date
                            fund_parallel_edge_node_pairs[e_type][k]['end_date'] = late_date
...
def aggregate_same_edges(g, label, threshold, **kwargs):
    ...
    if object_of_interest is not None:
        node_list = [x for x in node_list if x != object_of_interest]  # never aggregate the object of interest
...
if __name__ == "__main__":
    ...
    max_fund_depth = 2
    max_membership_depth = 2
    max_communication_depth = 2
    ...
    peeps = [(x.match_name, 'Person') for x, y in people_with_memberships]
    orgos = [(x.match_name, 'Organization') for x in all_organizations]
    ...
    for tup in build_graph_for:
        ...
        for com in combinations:
            ...
            if do_snap:
                test_graph = nx.snap_aggregation(temp_tot_graph, node_attributes=['type'], edge_attributes=['type'], )
                ...
                if ("aggregate_same_edges", agg_threshold) not in reductions:
                    ...
                    if ('aggregate_same_edges_person', agg_threshold) not in reductions:
                        ...
                        temp_tot_graph = aggregate_same_edges(temp_tot_graph, "Other Parties", agg_threshold,
                                                              object_of_interest_mn=poi_mn, node_type="Person")
                        tot_nodes = nx.number_of_nodes(temp_tot_graph)
                        reductions.append(("aggregate_same_edges_person", agg_threshold))
                        continue

                    if ('aggregate_same_edges_organizations', agg_threshold) not in reductions:
                        temp_tot_graph = aggregate_same_edges(temp_tot_graph, "Other Orgs", agg_threshold,
                                                              object_of_interest_mn=poi_mn, node_type="Organization")
                        tot_nodes = nx.number_of_nodes(temp_tot_graph)
