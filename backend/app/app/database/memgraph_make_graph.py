...
from gqlalchemy import query_builder
...
def mapped_memgraph_to_nx(res_dict):
    ...
    for n in nodes:
        ...
        g.add_node(n.properties['match_name'], display_name=n.properties["display_name"], type=lbl, value=n_val)

    for e in edges:
        if e.type == "COMMUNICATION":
            g.add_edge(node_dict[e.start_id].properties['match_name'], node_dict[e.end_id].properties['match_name'],
                       type=e.type,
                       date=e.properties['com_date'], amount=1, dash=[2, 3], color='red')
        elif e.type == 'FUNDS':
            g.add_edge(node_dict[e.start_id].properties['match_name'], node_dict[e.end_id].properties['match_name'],
                       type=e.type,
                       start_date=e.properties['start_date'], end_date=e.properties['end_date'],
                       amount=e.properties['amount'], dash=[2, 3], color='green')
        else:

            g.add_edge(node_dict[e.start_id].properties['match_name'], node_dict[e.end_id].properties['match_name'],
                       start_date=e.properties.get('start_date', early_date),
                       end_date=e.properties.get('end_date', late_date),
                       type=e.type, color='black', amount=1, dash=[0, 0]
                       )
...
def memgraph_to_nx(res_list):
    ...
    for row in res_list:
        ...
        if row['r'].type == "COMMUNICATION":
            g.add_edge(row['n'].display_name, row['m'].display_name, type=row['r'].type,
                       date=row['r'].com_date, amount=1, dash=[2, 3], color='red')
        elif row['r'].type == 'FUNDS':
            g.add_edge(row['m'].display_name, row['n'].display_name, type=row['r'].type,
                       start_date=row['r'].start_date, end_date=row['r'].end_date,
                       amount=row['r'].amount, dash=[2, 3], color='green')
        else:
            g.add_edge(row['n'].display_name, row['m'].display_name, type=row['r'].type, start_date=row['r'].start_date,
                       end_date=row['r'].end_date, amount=1, dash=[0, 0],
                       color='black'
                       )
...
def aggregate_parallel_edges(g):
    ...
    # TODO handle the cases where we feed this function edges of different types
    # get parallel edges
    fund_parallel_edge_node_pairs = {}
        ...
        if len(adj_mat[src]) > 0:
            for dst in adj_mat[src].keys():
                if len(adj_mat[src][dst]) > 1:  # this will be the parallel edge
                    ...
                    k = (src, dst)
                    if k not in fund_parallel_edge_node_pairs.keys():
                        fund_parallel_edge_node_pairs[k] = {'amount': 0}

                    early_date = date(2100, 1, 1)
                    late_date = date(1900, 1, 1)
                    for r in adj_mat[src][dst].keys():
                        fund_parallel_edge_node_pairs[k]['amount'] += adj_mat[src][dst][r]['amount']

                        if 'com_date' in adj_mat[src][dst][r].keys():
                            if adj_mat[src][dst][r]['com_date'] < early_date:
                                early_date = adj_mat[src][dst][r]['com_date']
                            if adj_mat[src][dst][r]['com_date'] > late_date:
                                late_date = adj_mat[src][dst][r]['com_date']

                        elif 'start_date' in adj_mat[src][dst][r].keys():
                            sd = adj_mat[src][dst][r]['start_date']
                            ed = adj_mat[src][dst][r].get('end_date', sd)
                            # TODO optimize order
                            if sd < early_date:
                                early_date = sd
                            elif sd > late_date:
                        ...
                        fund_parallel_edge_node_pairs[k]['start_date'] = early_date
                        fund_parallel_edge_node_pairs[k]['end_date'] = late_date

    # merge the parallel edges
    for k in fund_parallel_edge_node_pairs.keys():
        # get edges
        edges = g.get_edge_data(k[0], k[1])
        ...
        for e in edges.keys():
            if e_type != edges[e]['type'] or dash != edges[e]['dash'] or color != edges[e]['color']:
                raise RuntimeError("inconsistent edges being merged")

        # create the data for the new edge
        new_edge = {'type': e_type,
                    'dash': dash,
                    'color': color,
                    'amount': fund_parallel_edge_node_pairs[k]['amount'],
                    "start_date": fund_parallel_edge_node_pairs[k]['start_date'],
                    'end_date': fund_parallel_edge_node_pairs[k]['end_date']}
...
def aggregate_leaves(g, label, threshold):
    ...
    early_date = date(2100, 1, 1)
    late_date = date(1900, 1, 1)
    for leaf in funding_leaves:  # these are leaves, so they will only ever have one target
...
def aggregate_same_edges(g, label, threshold):
    ...
    for n in list(g.nodes):
        if g.nodes[n]['type'] == "Person":  # skip organizations
            edges[n] = adj_edge_summary(g.adj[n])

    non_empty_sets = {n: s for n, s in edges.items() if len(s) > 0}
    unique_combinations = list(set([s for n, s in non_empty_sets.items()]))

    binned_nodes = {i: [n for n, s in non_empty_sets.items() if s == uc] for i, uc in enumerate(unique_combinations)}
...
if __name__ == "__main__":
    ...
    max_fund_depth = 3
    max_membership_depth = 3
    max_communication_depth = 3
    target_max_nodes = 300
    start_agg_threshold = 100
    ...
    combinations = itertools.product(r_membership_depth, r_communication_depth, r_fund_depth)
    ...
    for com in combinations:
        ...
        query_funds_line_1 = f'MATCH p=(n: MGPerson {{match_name: "{poi_mn}"}}) - [l:FUNDS *..1] - (m)'
        query_membership_line_1 = f'MATCH p=(n: MGPerson {{match_name: "{poi_mn}"}}) - [l:MEMBERSHIP *..1] - (m)'
        query_communications_line_1 = f'MATCH p=(n: MGPerson {{match_name: "{poi_mn}"}}) - [l:COMMUNICATION *..1] - (m)'
        ...
        while tot_nodes > target_max_nodes and not brk:
            ...
            if ('aggregate_membership_leaves', agg_threshold) not in reductions:
                temp_membership_g = membership_g.copy(as_view=False)
                temp_membership_g = aggregate_leaves(temp_membership_g, 'Other Members', agg_threshold)
            ...
            if ('aggregate_communication_leaves', agg_threshold) not in reductions:
                temp_communication_g = communication_g.copy(as_view=False)
                temp_communication_g = aggregate_leaves(temp_communication_g, "Other Parties", agg_threshold)
            ...
            if ('aggregate_same_edges', agg_threshold) not in reductions:
                temp_tot_graph = create_tot_graph(temp_membership_g, temp_fund_g, temp_communication_g)
                temp_tot_graph = aggregate_same_edges(temp_tot_graph, "Other Parties", agg_threshold, )
