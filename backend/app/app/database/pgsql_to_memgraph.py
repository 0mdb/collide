...
class MGPerson(Node, db=db):
    id: Optional[int] = Field(exists=True, db=db)
    display_name: Optional[str] = Field(exists=True)
    match_name: Optional[str] = Field(exists=True)
...
class MGOrganizationType(Node, db=db):
    ...
    match_name: Optional[str] = Field()


class MGOrganization(Node, db=db):
    id: Optional[int] = Field(exists=True)
    display_name: Optional[str] = Field(exists=True)
    match_name: Optional[str] = Field(exists=True)
...
if __name__ == "__main__":
    ...
    index_command_start = "CREATE INDEX ON"
    index_tags = [
        ":MGPerson;",
        ":MGPerson(id);",
        ":MGPerson(match_name);",
        ":MGOrganization;",
        ":MGOrganization(id);",
        ":MGOrganization(match_name);",
    ]

    for tag in index_tags:
        cmd = " ".join([index_command_start, tag])
        blogger(cmd)
        db.execute(cmd)

    unique_command_start = "CREATE CONSTRAINT ON"
    unique_tags = [
        "(n:MGPerson) ASSERT n.id IS UNIQUE;",
        "(n:MGPerson) ASSERT n.display_name IS UNIQUE",
        "(n:MGPerson) ASSERT n.match_name IS UNIQUE;",
        "(n:MGOrganization) ASSERT n.id, n.match_name IS UNIQUE;",
        "(n:MGOrganization) ASSERT n.display_name IS UNIQUE;",
        "(n:MGOrganization) ASSERT n.match_name IS UNIQUE;",
    ]

    for tag in unique_tags:
        cmd = " ".join([unique_command_start, tag])
        db.execute(cmd)
    ...
    if input_org_org_funding:
        ...
        for f in progressbar.progressbar(person_person_funding):
            try:
                p1 = MGOrganization(id=f.party_1).load(db=db)
                p2 = MGOrganization(id=f.party_2).load(db=db)
