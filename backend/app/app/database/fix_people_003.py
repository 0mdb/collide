    ...
    if nt[0] in first_name_synonyms.keys():
        ...
        for name in first_name_synonyms[nt[0]]:
            ...
            for q in potential_names:
                ...
                sql_query = select(Person).where(Person.match_name == create_match_name(q))
                res = sess.exec(sql_query).all()
                if len(res) > 0:
                    ...
                    sql_query = select(Person).where(Person.match_name == create_match_name(name_to_go_with))
                    keep_person = sess.exec(sql_query).first()

                    print(f"we are going to keep {keep_person.display_name} (id {keep_person.id})")

                    for alt_first_name in first_name_synonyms[most_proper_first_name[0]]:
                        other_name = " ".join([alt_first_name] + nt[1:])
                        sql_query = select(Person).where(Person.match_name == create_match_name(other_name))
                        res = sess.exec(sql_query).first()
                        if res is not None:
                            print(f"\twe are going to get rid of {res.display_name} (id {res.id})")

                            sql_query = select(OrganizationMembership).where(OrganizationMembership.person == res.id)
