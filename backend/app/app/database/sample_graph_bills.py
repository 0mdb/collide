...
for idx, each_stage_hash in enumerate(stage_listing[:-1]):
    nodes_links_dict["links"].append({
        "source": stage_listing[idx],
        "target": stage_listing[idx+1],
...
for idx, each_stage_hash in enumerate(stage_listing):
    ...
    if len(res) == 1:
        ...
        for each_individual_vote in res:
            ...
            stat = select(Communications).where(
                Communications.party_1 == person_id or Communications.party_2 == person_id
            ).where(
            ...
            for each_communication in res:
                ...
                other_person_hash = hash((other_person_id, each_communication.com_date))
